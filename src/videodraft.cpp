...
int main(int argc, char *argv[]) {
    ...
    string VideoName = parser.get<string>("filename");
    string FullFilename = Path + VideoName;
    string WindowName = "Video Image";
    ...
    cout << "Saving output video with the following:" << endl;
    cout << "Video: " << Path + VideoName << endl;
    cout << "CODEC: " << " 0x" << std::hex << std::uppercase << CODEC << std::nouppercase << std::dec << ", " << CODEC << ", " << string(Char_CODEC) << endl;
    ...
    while (true) {
        ...
        if (videoOn) {
            ...
            if (!videoisOpen) {
                // setup video file
                Video.open(Path + VideoName, CODEC, FramesPerSecond, colour_disp.size(), isColor);
                if (!Video.isOpened()) {
                    cout  << "Could not open the video file for writing." << endl;
                    return -1;
                } else {
                    cout << "Video file is open" << endl;
                    videoisOpen = true;
                }
            }
